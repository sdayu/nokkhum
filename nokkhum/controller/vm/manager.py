...
class VMManager(object):
    ...
    def acquire(self):
        ...
        if compute_nodes:
            for compute_node in compute_nodes:
                if compute_node.vm.status == 'pending':
                    logger.debug("VM id: %s ip: %s is in wait time"
                                 % (compute_node.vm.instance_id,
                                    compute_node.vm.ip_address))
                    time.sleep(10)
                    return
        else:
        ...
        while status == 'pending':
            logger.debug("compute node id: %s vm instance_id: %s is pending"
                         % (compute_node.id, instance.id))
            time.sleep(1)
            status = instance.update()
        ...
        if status == 'running':
            compute_node.vm.extra['first_running_date'] = \
    ...
    def start(self, instance_type=None):
        ...
        vm_info.started_instance_date = datetime.datetime.now()
        ...
        compute_node.save()
        compute_node.reload()
        logger.debug("compute node id: %s vm instance_id: %s is starting"
                     % (compute_node.id, instance.id))
    ...
    def terminate(self, instance_id):
        ...
        compute_node = models.ComputeNode.objects(
            vm__instance_id=instance_id).first()
