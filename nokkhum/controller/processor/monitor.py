...
class ProcessorMonitoring(threading.Thread):
    ...
    def __request_new_processor_command(self, processor, action, message):
        processor_command = models.ProcessorCommand.objects(
            processor=processor, action=action, status='waiting').first()
        ...
        for processor in processors:
            if processor.operating.user_command == "run":
                if processor.operating.status == "running" \
                        or processor.operating.status == "starting":
                    diff_time = current_time - processor.operating.updated_date
                    logger.debug("processor id: %s diff: %d s"
                                 % (processor.id, diff_time.total_seconds()))
                    if diff_time > datetime.timedelta(
                            seconds=self.maximum_wait_time):
                        logger.debug("processor id: %s disconnect diff: %d s"
                                     % (processor.id,
                                        diff_time.total_seconds()))
                        message = "Processor disconnect.\n Restart processor by ProcessorMonotoring: %s"\
                            % datetime.datetime.now()
                        self.__request_new_processor_command(processor,
                                                             "start", message)
                elif processor.operating.status == "fail":
                    logger.debug("processor id: %s status fail" % processor.id)
                    message = "Processor run fail.\n "
                    message += "Restart processor by ProcessorMonotoring: %s"\
                        % datetime.datetime.now()
                    self.__request_new_processor_command(processor, "start",
                                                         message)
