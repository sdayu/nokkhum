...
#        self._consumer = consumer.ConsumerFactory().get_consumer(routing_key)
        ...
        if 'method' not in body:
            ...
            if 'message_id' in body:
                ...
                if body['method'] == 'list_processors':
                    ...
                    respons = self.processor_controller.get_processor_attributes(
                        body['args']['processor_id'])
                elif body['method'] == 'start_processor':
                    respons = self.processor_controller.start_processor(
                        body['args']['processor_id'], body['args']['attributes'])
    ...
    def reconnect_message_connection(self):
        ...
        ip = "127.0.0.1"
        try:
            ip = netifaces.ifaddresses(self.configurator.settings.get(
        ...
        self.update_publisher = connection.Connection.get_instance(
        ).publisher_factory.get_publisher("nokkhum_compute.update_status")
        self.rpc = connection.Connection.get_instance(
        ).get_rpc_factory().get_default_rpc_server(ip)
