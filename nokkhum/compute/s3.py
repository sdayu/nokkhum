...
#            print file_name[prefix_length:]
    ...
    def __list_local_file(self, prefix_dir, file_list):
        extension_filter = [
            "avi", "png", "jpg", "ogv", "ogg", "mp4", "webm", "webp"]
...
class S3Thread(threading.Thread):
    ...
    def __init__(self):
        ...
        access_key_id = config.Configurator.settings.get(
            'nokkhum.storage.s3.access_key_id')
        secret_access_key = config.Configurator.settings.get(
        ...
        secure = config.Configurator.settings.get(
    ...
    def run(self):
        ...
        prefix_dir = config.Configurator.settings.get(
            'nokkhum.processor.record_path')
