...
class RPC:
    ...
    def initial(self):
        self._publisher = publisher.PublisherFactory(
            self.channel).get_publisher("nokkhum_compute.*.rpc_request")
        self._consumer = consumer.ConsumerFactory(self.channel).get_consumer(
            "nokkhum_compute.127:0:0:1.rpc_response")
...
class RpcClient(RPC):
    ...
    def initial(self):
        self._publisher = publisher.PublisherFactory(
            self.channel).get_publisher(self._publisher_rounting_key)
        self._consumer = consumer.ConsumerFactory(
            self.channel).get_consumer(self._consumer_rounting_key)
        self.regist_default_consumer_callback()
...
class RpcServer(RPC):
    ...
    def __init__(self, channel, ip):
        self._publisher_rounting_key = "nokkhum_compute.*.rpc_response"
