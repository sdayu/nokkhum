...
class ReportTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not os.path.exists(config.Configurator.settings["nokkhum.processor.record_path"]):
            os.mkdir(config.Configurator.settings["nokkhum.processor.record_path"])
    ...
    def test_system_report(self):
        ...
        self.assertGreaterEqual(resource["cpu"]["used"], 0, "cpu use less than 0")
        self.assertGreaterEqual(resource["memory"]["used"], 0, "cpu use less than 0")
        self.assertGreaterEqual(resource["disk"]["used"], 0, "cpu use less than 0")
